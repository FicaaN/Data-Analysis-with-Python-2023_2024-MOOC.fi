...
def cleaning_data():
    ...
    df["President"] = (
        df["President"].str.split(",").map(lambda x: " ".join(x[::-1])).str.strip()
    )

    # This one also does the same like for the presidents
    df["Vice-president"] = (
        df["Vice-president"]
        .str.split(",")
    ...
    df["Seasons"] = df["Seasons"].map(
        lambda x: 2 if x == "two" else (int(x) if x != "-" else np.nan)
    )
